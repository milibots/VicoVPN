#!/usr/bin/env python3
r"""
VicoVPN responsive navbar and collision-fix patch.

Run from:
    C:\AndroidProjects\VicoVPN

Commands:
    python apply_vicovpn_responsive_nav_collision_fix.py
    python apply_vicovpn_responsive_nav_collision_fix.py --install
    python apply_vicovpn_responsive_nav_collision_fix.py --no-build
    python apply_vicovpn_responsive_nav_collision_fix.py --rollback
...
BACKUP_ROOT = (
    ROOT
    / ".vicovpn_responsive_nav_backups"
)

STATE_FILE = (
    ROOT
    / ".vicovpn_responsive_nav_state.json"
)
...
def replace_header(text):
    status_bounds = element_bounds_by_id(
        text,
        "statusIconCard",
    )
    ...
    if status_bounds is None:
        fail(
            "statusIconCard was not found."
        )

    header_bounds = enclosing_element(
        text,
        status_bounds[0],
        "FrameLayout",
    )
    ...
    if header_bounds is None:
        fail(
            "Connection header container was not found."
        )
    ...
    if (
        ...
        fail(
            "The detected block is not the connection header."
        )
...
def run_gradle(install):
    ...
    if result.returncode != 0:
        ...
        print(
            "Rollback with:\n"
            "  python "
            "apply_vicovpn_responsive_nav_collision_fix.py "
            "--rollback",
            file=sys.stderr,
        )
